...
//oszczedzmy sobie poprawiania
vec3 Normal0 = iData.normal;
vec3 WorldPos0 = iData.worldPos.xyz;
...
{ 
   //vec3 Normal = normalize(Normal0);
    ...
    for (int i = 0 ; i < gNumSpotLights ; i++) {
 	...
 	outputColor = vec4(iData.colour,1) * TotalLight;
//	FragColor = vec4(iData.WorldPos,1);
	//fog calculation
	float z = gl_FragCoord.z / gl_FragCoord.w;
	float fog = clamp(exp(-fogdensity * z * z), 0.2, 1);
	outputColor = mix(fogcolor, outputColor, fog);	
	//outputColor = theColor;
	//outputColor = vec4(iData.worldPos.xyz,1);
}