...
type
  ...
  private
    ...
    nodeMap:TDictionary<integer,tbehTreeNode>;
    StartTime:cardinal;
...
function saveNodes(b:tbehtreenode;n:ixmlnode;var id:integer):ixmlnode;
var
  k:tbehtreenode;
  me:ixmlnode;
  i:integer;
begin
  i:=0;
  me:=n.AddChild('item'+inttostr(id));
  inc(id);
  ...
  for i:=0 to b.count-1 do saveNodes(b.getKid(i),me,id);
end;
...
begin
  ...
  id:=0;
  saveNodes(n,xml.AddChild('Root'),id);
...
begin
  s:=n.NodeName;
  if length(s)=4 then i:=strtoint(formeditor.getnextselectorid)
  else begin
       i:=strtoint(copy(s,5,length(s)-4));
       if formeditor.selectorID<= i then
          formeditor.selectorID:=i+1;
  end;
  b.nodeID:=i;
  ...
  formEditor.nodeMap.Add(b.nodeID,b);
...
begin
 ...
 // if mymonth>3 then halt;
  ...
  nodeMap:=tDictionary<integer,TBehTreeNode>.create;
...
//       n.node.screenPos:=n.position.point- n.parentShit.Position.Point;
    nodes.Add(n);
    nodeMap.Add(n.node.nodeID,n.node);
  end else begin
...
begin
  ...
  if length(adata)=5 then begin
     ...
     status:=strtoint(msg[1]);
     id:=strtoint(copy(msg,2,4));
     if nodeMap.ContainsKey(id) then
        nodeMap[id].state:=nodeStates(status);
    // log(msg);
  end;
end;
