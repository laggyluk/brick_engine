...
uses
  Classes, SysUtils,math,fgl,core_listofrecords;

type


  TQueue = specialize TFPGList<coord_t>;
...
function astar_compute(const grid:pChar;solLength:pInteger;boundX, boundY, start,
		    eend:integer):pInteger;

function astar_unopt_compute (const grid:pChar;solLength:pInteger;boundX,boundY,
		    start,eend:integer):pInteger;


{ Compute cell indexes from cell coordinates and the grid width }
function astar_getIndexByWidth (width, x, y:integer):integer;

{ Compute coordinates from a cell index and the grid width }
procedure astar_getCoordByWidth ( width, node:integer;x,y:pInteger);
...
function estimateDistance (start, eend:coord_t):double;
begin
  	result:=max (abs (start.x - eend.x), abs (start.y - eend.y));
end;
  ...
  // all costs to 1
function preciseDistance(start, eend:coord_t ):double;
begin
  	if (start.x - eend.x <>  0) and( start.y - eend.y <> 0) then
  		result:= sqrt (power(start.x - eend.x, 2) +  power(start.y - eend.y, 2))
  	else
  		result:= abs (start.x - eend.x) + abs (start.y - eend.y);
end;
